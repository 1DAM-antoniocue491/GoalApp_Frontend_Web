...
export async function mockFetchTeamNextMatches(id: number): Promise<{
  ...
  const proximos = partidos
    .filter((p) =>
      ...
      (p.estado === 'programado' || p.estado === 'en_juego')
    )
 ...
 */
// IDs de rol según BD actual: 9=admin, 10=coach, 11=delegate, 12=player, 13=viewer
export async function mockFetchRoles(): Promise<MockRol[]> {
  await simulateDelay();
  return [
    { id_rol: 9, nombre: 'admin', descripcion: 'Administrador del sistema' },
    { id_rol: 10, nombre: 'coach', descripcion: 'Entrenador de equipo' },
    { id_rol: 11, nombre: 'delegate', descripcion: 'Delegado de partido' },
    { id_rol: 12, nombre: 'player', descripcion: 'Jugador de equipo' },
    { id_rol: 13, nombre: 'viewer', descripcion: 'Observador' },
  ];
}
...
export async function mockFetchUsersByLeague(ligaId: number): Promise<MockUserWithRole[]> {
  ...
  return usuariosLiga
    ...
    .map((u) => ({
      ...
      nombre: u.nombre,
      email: u.email,
      id_rol: u.roles[0]?.id_rol || 13,
      rol: roles[u.roles[0]?.id_rol || 13] as UserRole,
      activo: u.activo,
